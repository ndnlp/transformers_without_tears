...
class Generator():
    ...
    def beam_search(self, src, max_lengths, max_possible_length, decoder_one_step_fn, cache, beam_size):
        # first step, beam=1
        batch_size = src.size(0)
        tgt = torch.tensor([ac.BOS_ID] * batch_size).reshape(batch_size, 1) # [bsz, 1]
        next_token_probs = decoder_one_step_fn(tgt, 0, cache) # [bsz, V]
        
        # by default, do not allow EOS in first position
        if not self.args.allow_empty:
            next_token_probs[:, ac.EOS_ID] = float('-inf')
        ...
        not_eos_mask = (torch.arange(num_classes, device=self.device).reshape(1, -1) != ac.EOS_ID)
        ret = [None] * batch_size
        ...
        for time_step in range(1, max_possible_length + 1):
            ...
            finished_decoded = torch.sum((cumulative_symbols[:, :, -1] == ac.EOS_ID).type(max_lengths.type()), -1) == beam_size
            ...
            finished_mask = last_symbols.reshape(-1) == ac.EOS_ID
    ...
    def generate(self, src, src_lang_idx, tgt_lang_idx, logit_mask, beam_size):
        ...
        if decode_method == ac.SAMPLING:
            ...
            return self.beam_search(src, max_lengths, max_possible_length, decoder_one_step_fn, cache, beam_size)
